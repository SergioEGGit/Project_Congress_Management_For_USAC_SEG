...
class mensajeses extends Component {
        ...
        axios.get('http://localhost:4000/api/variables/1')
            .then(response => {
                ...
                axios.get('http://localhost:4000/api/peticionmensaje')
                    .then(response => {
                        console.log(response);
                        const arreglo = response.data.filter(d => d.peticion_r === variables.nombre_miembro && d.estado === "aprobada");
                        console.log(arreglo);
    ...
    peticion() {
        window.location.assign('http://localhost:3000/peticionmensaje');
    }

    render() {
        const {usuario_remitente, usuario_destinatario, asunto, cuerpo, usuarios, mensajes, asignacioncurso,peticionmensaje} = this.state;
        return (
            ...
            <div className="cont">
                ...
                <input name="buttoni" type="button" id="bbr12" onClick={this.peticion} value="Solicitud De Mensaje"/>
